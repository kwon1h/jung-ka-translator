...
| OCR 엔진 | [OCR엔진.md](features/OCR엔진.md) | [PaddleOCR(OpenVINO)](features/OCR엔진.md#paddleocropenvino), [모델 준비](features/OCR엔진.md#모델-준비), [품질 높이기](features/OCR엔진.md#ocr-품질을-높이는-방법) |
...
- OCR은 PaddleOCR(OpenVINO)로 고정되어 있으며 Windows 중국어·일본어 언어 팩을 설치할 필요가 없습니다. 처음 모델 준비가 실패하면 [모델 준비](features/OCR엔진.md#모델-준비)를 확인하세요.
