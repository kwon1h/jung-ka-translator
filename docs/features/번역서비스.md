...
![번역 서비스 설정 위치](../images/translator-service-settings.png)
...
3. `DeepL API 인증 키 (Free / Pro)` 입력란에 키를 붙여 넣습니다.
4. `저장`을 누릅니다.

앱은 키의 `:fx` 접미사를 확인해 Free 또는 Pro API 주소를 자동으로 선택합니다. DeepL API 키는 현재 Windows 사용자 계정 기준으로 보호 저장되며, 기존 버전과의 호환성을 위해 저장 파일 이름은 `%LOCALAPPDATA%\GameOverlayTranslator\deepl-free-auth.key`를 유지합니다.
...
설정값을 입력한 뒤 `연결 테스트`를 누르면 현재 선택한 번역 언어로 짧은 시험 문장을 한 번 번역합니다. 번역을 시작하기 전에 API 키, 웹 앱 URL, 네트워크 연결과 응답 형식을 확인할 수 있습니다. 입력한 DeepL 키와 Google Apps Script URL은 저장 전에도 테스트할 수 있습니다.
...
- [DeepL API 인증 및 Free/Pro 엔드포인트](https://developers.deepl.com/docs/getting-started/auth)
